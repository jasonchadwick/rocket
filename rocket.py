...
for j in range(1):  #goes from 750 to 2000
    ...
    while(True):
        ...
        while(running):
            ...
            force_turbulence = vector(random()-0.5, random()-0.5, random()-0.5) * 5000
            ...
            #calculates whether rocket needs to start slowing down or not
            ...
            if(rocket.y < 1):
                ...
                if(arrival_speed_squared > -((rocket.y) * 2)):
                    ...
                    if(rocket.theta > 0.05 and rocket.y < atm_height * 0.8):
                        if not(dot(rocket.ang_momentum, cross(rocket.axis, vector(0,1,0))) > 0):
                            righting_factor = 50
                            thrusting = True
                            info_label.text = "Correcting for angle"
                    else: thrusting = False

            if(rocket.fuel_pct <= 0): thrusting = False
            ...
            #changes thrust if it is angled too much (because engine cannot rotate past the limiting angle)
